# -*- coding: utf-8 -*-
# most of this is from https://github.com/foobar167/junkyard
...
class CanvasImage:
    ...
    def __init__(self, placeholder, path, onImageRightClick):
        ...
        self.canvas.bind('<ButtonPress-3>', self._mapClick)
    ...
    def __wheel(self, event):
        ...
        if event.num == 5 or event.delta == -120:  # scroll down, smaller
            if round(self.__min_side * self.imscale) < 250: return  # image is less than 250 pixels
    ...
    def getImageBBox(self):
        return self.canvas.coords(self.container)
        
    def _mapClick(self, event):
        x = self.canvas.canvasx(event.x)  # get coordinates of the event on the canvas
        y = self.canvas.canvasy(event.y)
    ...
    def markPlayerPos(self, pos, iconSize, color="red"):
        ...
        self.player = self.canvas.create_rectangle(pos[0]-size/2, pos[1]-size/2, pos[0]+size/2, pos[1]+size/2, fill=color)
        
    def markKelvinPos(self, pos, iconSize, color="blue"):
        size = iconSize * self.imscale
        if hasattr(self, "kelvin"): 
